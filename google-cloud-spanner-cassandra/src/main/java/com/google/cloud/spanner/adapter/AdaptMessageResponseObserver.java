...
class AdaptMessageResponseObserver implements ResponseObserver<AdaptMessageResponse> {
  ...
  public void onError(Throwable t) {
    ...
    try {
    channel.write(ByteBuffer.wrap(serverErrorResponse(streamId, t.getMessage())));
    } catch (IOException e) {
  ...
  public void onComplete() {
    ...
    try {
      if (collectedPayloads.isEmpty()) {
        channel.write(ByteBuffer.wrap(serverErrorResponse(streamId, "No response received.")));
        return;
      }

      final int numPayloads = collectedPayloads.size();

      channel.write(ByteBuffer.wrap(collectedPayloads.get(numPayloads - 1).toByteArray()));
        for (int i = 0; i < numPayloads - 1; i++) {
          channel.write(ByteBuffer.wrap(collectedPayloads.get(i).toByteArray()));
        }
    } catch (IOException e) {
      LOG.error("Error writing to socket for streamId: {}", streamId, e);
    }
