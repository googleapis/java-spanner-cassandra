  ...
  void start() {
    ...
    try {
      ...
      SessionPoolOptions sessionPoolOptions =
          SessionPoolOptions.newBuilder().setMinSessions(100).setMaxSessions(4000).build();
      Spanner spanner =
          SpannerOptions.newBuilder()
              .setProjectId("span-cloud-testing")
              .setNumChannels(options.getNumGrpcChannels())
              .setSessionPoolOption(sessionPoolOptions)
              .build()
              .getService();
      LOG.info("Using multiplexed sessions: " + sessionPoolOptions.getUseMultiplexedSession());

      dbClient =
          spanner.getDatabaseClient(
              DatabaseId.of("span-cloud-testing", "c2sp-load-test", "xobni_derived_2"));
